                ...
                <div class="card card-rounded border-0 shadow">
                  <div class="card-body p-5">
                    ...
                    <form method="POST" action="{{ route('register') }}">
                      @csrf
                      <label for="name" class="form-label text-gray-700 font-medium">{{ __('Name') }}</label>
                      <div class="mb-4">
                        <input type="text" class="form-control form-control-user shadow-sm @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" placeholder="{{ __('Nama') }}" required autocomplete="name" autofocus>
                        @error('name')
                      ...
                      <div class="mb-4">
                        <input type="email" class="form-control form-control-user shadow-sm @error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" placeholder="{{ __('Email') }}" required autocomplete="email">
                        @error('email')
                        <span class="invalid-feedback" role="alert">
                          <strong>{{ $message }}</strong>
                        </span>
                      ...
                      <label for="alamat" class="form-label text-gray-700 font-medium">{{ __('Alamat') }}</label>
                      <div class="mb-4 relative">
                        <textarea class="form-control form-control-user @error('alamat') is-invalid @enderror" name="alamat" cols="30" rows="10" placeholder="{{ __('Alamat') }}" required></textarea>
                        @error('alamat')
                        <span class="invalid-feedback" role="alert">
                          <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <label for="password" class="form-label text-gray-700 font-medium">{{ __('Password') }}</label>
                      <div class="mb-4 relative">
                        <input type="password" class="form-control form-control-user shadow-sm @error('password') is-invalid @enderror" name="password" placeholder="{{ __('Password') }}" required autocomplete="new-password">
